...
BIBLIO_CSV_TO_DB_CONVERTER = {
    'id': ('id', int),
    'ref_id': ('ref_id', unicode),
    'title': ('title', unicode),    
    'link': ('link', unicode),
}
...
class Reference(object):
    """A class to represent an entry to the bibliography."""
    ...
    def list_properties(self, name, output):
        ...
        output.append('%s[\'id\'] = %s;' % (name, self.id))
        output.append('%s[\'ref_id\'] = \'%s\';' % (
            name, escape_quote(self.ref_id)))
        output.append('%s[\'title\'] = \'%s\';' % (
            name, escape_quote(self.title)))
        output.append('%s[\'note_link\'] = \'%s\';' % (
            name, escape_quote(self.note_link)))
...
class Verifier(object):
    ...
    def load_and_verify_model(self, echo_func):
        ...
        biblio_file = os.path.join(os.path.dirname(__file__), '../data/biblio.csv')
        ...
        lessons = read_objects_from_csv_file(lesson_file, LESSONS_HEADER, Lesson)
        self.info('Read %s lessons' % len(lessons))
        
        self.info('Loading references from: %s' % biblio_file)
        references = read_objects_from_csv_file(biblio_file, BIBLIO_HEADER, Reference)
        self.info('Read %s references' % len(references))

        self.verify_unit_fields(units)
        self.verify_lesson_fields(lessons)
        self.verify_unit_lesson_relationships(units, lessons)
        # TODO - DER: Similar rules to 'verify_unit_fields'
        #self.verify_reference_fields(references)
